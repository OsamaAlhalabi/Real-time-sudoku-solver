...
Author: Paul Moon
Date: December 2012
Modified by: Nitro963
Modification Date: December 2020
...
def stringify(s):
    ...
    for i in range(9):
        for j in range(9):
            output = ''.join([output, str(s[i][j])])

    return ''.join([output, '\n'])


def check_valid(s, row, col):
    """
...
def populate_board(s, row, col):
    ...
    for i in range(9):
        s[row][col] = temp[i]
        if check_valid(s, row, col):
            if populate_board(s, row, col + 1):
                return True
    s[row][col] = 0
    return False
